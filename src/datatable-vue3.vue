...
export default /*#__PURE__*/ defineComponent({
  name: "DatatableVue3", // vue component name
  components: {
    Skeletor,
  },
  data(): IDataTableData {
      ...
      if (this.sortable !== false)
        computedRows = computedRows.sort((x: any, y: any) => {
          ...
          const cook = (x: any) => {
            x = this.collect(
              x,
              (this.columns as IColumn[])[this.sortColumn].field
            );
            if (typeof x === "string") {
              x = x.toLowerCase();
              if ((this.columns as IColumn[])[this.sortColumn].numeric)
                x = x.indexOf(".") >= 0 ? parseFloat(x) : parseInt(x);
            }
            return x as readonly string[];
          };
          x = cook(x);
          y = cook(y);
          return (
  ...
  methods: {
    ...
    onTableLength(e: Event) {
      this.currentPerPage = parseInt((e.target as HTMLInputElement).value);
    },
    ...
    print(): void {
      const clonedTable = (this.$refs.table as HTMLElement).cloneNode(
        true
      ) as HTMLElement;
      this.synchronizeCssStyles(
      ...
      (clonedTable.querySelector(".actions") as HTMLElement).remove();
      (
        clonedTable.querySelector(".material-pagination") as HTMLElement
      ).remove();
      (clonedTable.querySelector(".datatable-length") as HTMLElement).remove();
      clonedTable
    ...
    renderTable(): string {
      ...
      table += "<tr>";
      for (let i = 0; i < (this.columns as IColumn[]).length; i++) {
        const column = this.columns[i] as IColumn;
        table += "<th>";
        table += column.label;
        table += "</th>";
      }
      ...
      for (let i = 0; i < this.rows.length; i++) {
        const row = this.rows[i] as IRow[];
        table += "<tr>";
        for (let j = 0; j < this.columns.length; j++) {
          const column = this.columns[j] as IColumn;
          table += "<td>";
    ...
    dig(obj: string, selector: string): string | readonly string[] | undefined {
      let result = obj;
      const splitter = selector.split(".") as unknown as number[];
    ...
    synchronizeCssStyles(
      ...
      if (recursively) {
        ...
        for (var i = vSrcElements.length; i--; ) {
          ...
          (vDstElement as HTMLElement).style.cssText =
            this.getComputedStyleCssText(vSrcElement);
        }
      }
    },
    getComputedStyleCssText(element: Element) {
...
table th.sorting-asc:after {
  ...
  word-wrap: normal;
  -moz-webkit-font-feature-settings: "liga";
